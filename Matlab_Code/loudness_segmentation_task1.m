...
for i = 1:length(audioFiles)
    ...
    for j = 1:length(words)
        ...
        rms_values(j) = sqrt(mean(segment.^2)); % RMS for each segment
    end

    rms_threshold = mean(rms_values);
    
    %Finding louder words: Only words with 1 in loudness_indicator are considered
    louder_indices = find(loudness_indicator == 1 & rms_values > rms_threshold);
    
    %highlighting the louder segments in the audio (finding their time ranges)
    ...
    fprintf('Avg RMS for file %s: %.4f\n', textFiles(i).name, rms_threshold);
